...
function ListGroup({ items, heading, onSelectItem }: Props) {
  ...
  return (
    <>
      <h1>{heading}</h1>
      {items.length === 0 && <p>No item found</p>}
      <ul className={[styles.listGroup, styles.container].join(' ') // [].join(' ') : A Great way to have multiple classes!
    }> 
        {items.map((item, index) => (
          <li
            className={
...
import styled from 'styled-components' // npm i @types/styled-components
//import styles from "./ListGroup.module.css" // Now styles is just a regular JS object that has all the classes we have defined in this file
